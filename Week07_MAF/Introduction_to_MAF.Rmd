...
date: 07/17/22
updated: 9/29/2025
Milo Gittings (worked with Haley as partner)
---
...
```
```{r}
clinical <- read.csv("/home1/gittings/490_cluster/analysis_data/brca_clinical_data.csv")
```
```{r}
maf_query <- GDCquery(
  project = 'TCGA-BRCA',
...
setwd("/home1/gittings/490_cluster/analysis_data")
...
```{r}
saveRDS(maf_object, file = "/home1/gittings/490_cluster/analysis_data/maf_object.rds")
...
```{r}
maf_object <- readRDS(file = "/home1/gittings/490_cluster/analysis_data/maf_object.rds")
...
coOncoplot(m1 = er_pos_maf, 
           ...
           borderCol = NA)
```
There is a significant difference in the TP53 gene being mutated 21% in ER positive patients and 81% in ER negative
...
print(fisher_test)
```
The p value is much less than 0.05 and therefore means that the relation between ER status and a mutation in the TP53 gene is very statistically significant
Having a mutated TP53 gene is 16.45x more likely in negative ER patients compared to + er patients
...
lollipopPlot2(m1 = er_pos_maf,
              ...
              gene = "TP53")
```
Between the ER Negative and ER Positive groups, there is a difference even though mutation are on the p53 gene for both. The ER negaitve group seems to have much more and many different types of mutations across the gene. 
