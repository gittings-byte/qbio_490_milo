...
rownames(mtcars)
colnames(mtcars)
print("Horsepower for Duster 360, Lincoln Continental and Maserati Bora")
mtcars[c(7, 16, 31), "hp"]
```
...
```{r}
write.csv(clinic, "/home1/gittings/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/gittings/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
head(clinic)
str(clinic)
# 1) There are 6 rows, 112 columns
# 2) It says "chr" data type
clinic[5, "birth_days_to"] #3) 18371 Days, 50Years old
pat1age <- clinic[1, "birth_days_to"] 
pat1lastcont <- clinic[1, "last_contact_days_to"]
as.numeric(pat1age) + as.numeric(pat1lastcont)
# 4) 16444 days, 45 years old
```
...
```{r Exercise 4.2}
# create plot here
clinic$dx_year <- as.numeric(clinic$initial_pathologic_dx_year)
clinic$cent17 <- as.numeric(clinic$cent17_copy_number)
clean_data <- clinic[!is.na(clinic$dx_year) & !is.na(clinic$cent17), ]
clean_data$dx_year <- as.factor(clean_data$dx_year)
boxplot(
  cent17 ~ dx_year,
  data = clean_data,
  xlab = "Initial Pathologic Diagnosis Year",
  ylab = "Centromere 17 Copy #",
  main = "Centromere 17 Copy # by Diagnosis Yr",
  col = "lightcoral",
  las = 2)
```
...
```{r}
no_middle_clinic <- under30_over60_mask
```
...
```{r}
cleaned_clinic$age_category <- ifelse(
  cleaned_clinic$age_at_diagnosis <= 35, "Young", ifelse(
    cleaned_clinic$age_at_diagnosis < 50, "Middle", "Old"
    )
  )
```
...
# if it is not applicable, use last contact days, and if not, use death days to 
```
...
Survival probability as a percent

# 2. What does the x axis represent?
Time in days

# 3. What age group appears to have the greatest rate of survival?
It appears that the middle category has the greatest rate of survival. But with more thought
it is the younger age category, and the scarce data points could indicate survival or treatment
...
15 years is 5475 days, which equates to about a 30% survival rate
...
The p value is 0.2, which is way bigger than 0.05, meaning that the data comparing age and breast
cancer survival is NOT statistcially significant
